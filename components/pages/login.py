...
def _ico_arrow_right() -> FT:
    return Svg(NotStr('<polyline points="9 18 15 12 9 6"/>'),
               xmlns="http://www.w3.org/2000/svg", viewBox="0 0 24 24", fill="none",
               stroke="currentColor", stroke_width="2.5", stroke_linecap="round",
               stroke_linejoin="round", width="16", height="16")
...
def _progress_bar(step: int) -> FT:
    labels = ["Your details", "Verify email"]
    return Div(cls="wz-progress")(
        ...
        Div(cls="wz-step-label")(
            Span(f"Step {step} of 2", cls="wz-step-num"),
            Span(" — ", cls="wz-sep"),
            Span(labels[step - 1], cls="wz-step-name"),
        ),
        Div(cls="auth-progress-track")(
            Div(cls="auth-progress-fill", style=f"width:{int(step / 2 * 100)}%"),
        ),
    )
...
# ─── Step 0 — Sign In / Sign Up choice ────────────────────────────────────────

def _choice_step() -> FT:
    return Div(cls="wz-step", id="wz-choice")(
        Div(cls="wz-choice-header")(
            Div(cls="wz-hero-icon")("🛡️"),
            H1("Welcome to Teluka", cls="auth-title"),
            P("Safe marketplace deals for the Philippines — powered by escrow.",
              cls="auth-sub"),
        ),
        Div(cls="wz-choice-row")(
            Button(
                Div(cls="wz-choice-icon")("👤"),
                Div(cls="wz-choice-body")(
                    Div("Sign In", cls="wz-choice-title"),
                    Div("Already have an account", cls="wz-choice-desc"),
                ),
                _ico_arrow_right(),
                cls="wz-choice-card",
                type="button",
            ...
            Button(
                Div(cls="wz-choice-icon")("✨"),
                Div(cls="wz-choice-body")(
                    Div("Sign Up", cls="wz-choice-title"),
                    Div("Create a free account", cls="wz-choice-desc"),
                ),
                _ico_arrow_right(),
                cls="wz-choice-card wz-choice-card-signup",
                type="button",
                onclick="showSignUp()",
            ),
        ),
        Script("""
function showSignIn() {
  document.getElementById('wz-choice').style.display = 'none';
  var el = document.getElementById('wz-signin');
  if (el) { el.style.display = ''; el.style.animation = 'auth-enter 0.35s cubic-bezier(0.16,1,0.3,1) both'; }
  var ph = document.getElementById('si-ph-input');
  if (ph) setTimeout(function(){ ph.focus(); }, 50);
}
function showSignUp() {
  document.getElementById('wz-choice').style.display = 'none';
  var el = document.getElementById('wz-signup');
  if (el) { el.style.display = ''; el.style.animation = 'auth-enter 0.35s cubic-bezier(0.16,1,0.3,1) both'; }
  var ph = document.getElementById('su-ph-input');
  if (ph) setTimeout(function(){ ph.focus(); }, 50);
}
function showChoice() {
  var si = document.getElementById('wz-signin');
  var su = document.getElementById('wz-signup');
  if (si) si.style.display = 'none';
  if (su) su.style.display = 'none';
  var ch = document.getElementById('wz-choice');
  if (ch) { ch.style.display = ''; ch.style.animation = 'auth-enter 0.35s cubic-bezier(0.16,1,0.3,1) both'; }
}
        """),
    )
...
def _signin_step(error: str | None = None) -> FT:
    return Div(cls="wz-step", id="wz-signin", style="display:none")(
        _progress_bar(1),

        Div(cls="wz-step-header")(
            Div(cls="wz-hero-icon")("👤"),
            H1("Sign in", cls="auth-title"),
            P("Enter your registered phone number.", cls="auth-sub"),
        ),

        Div(id="si-phone-wrap")(
            Form(
                ...
                Div(cls="form-group")(
                    ...
                    Div(cls="input-icon-wrap", id="si-ph-wrap")(
                        Span("+63", cls="input-prefix"),
                        Input(
                            type="tel", name="identifier", id="si-ph-input",
                            placeholder="917 123 4567",
                            cls="form-input has-prefix",
                            autocomplete="tel", inputmode="numeric",
                            oninput="validatePhone(this,'si-ph-wrap')",
                        ),
                ...
                Button(
                    Span("Continue"), Span(cls="htmx-indicator"),
            ...
            Div(cls="wz-alt-row")(
                ...
                Button("Sign in with email →", cls="wz-alt-link",
                       type="button", onclick="showSiEmail()"),
            ),
        ),

        Div(id="si-email-wrap", style="display:none")(
            Form(
                hx_post="/check-identifier",
                hx_target="#auth-step",
                hx_swap="innerHTML",
                ...
                Div(cls="form-group")(
                    ...
                    Div(cls="input-icon-wrap", id="si-em-wrap")(
                        Div(cls="input-icon")(_ico_email()),
                        Input(
                            type="email", name="identifier", id="si-em-input",
                            placeholder="you@gmail.com",
                            cls="form-input",
                            autocomplete="email",
                            oninput="validateEmail(this,'si-em-wrap')",
                        ),
                        Div(cls="input-valid-icon")(_ico_check()),
                    ),
                ),
            ...
            Button("← Use phone instead", cls="wz-back-link",
                   type="button", onclick="showSiPhone()"),
        ),

        Div(cls="wz-nav-row")(
            Button("← Back", cls="wz-back-link", type="button",
                   onclick="showChoice()"),
        ),

        Script("""
function showSiEmail() {
  document.getElementById('si-phone-wrap').style.display = 'none';
  document.getElementById('si-email-wrap').style.display = '';
  var el = document.getElementById('si-em-input'); if (el) el.focus();
}
function showSiPhone() {
  document.getElementById('si-email-wrap').style.display = 'none';
  document.getElementById('si-phone-wrap').style.display = '';
  var el = document.getElementById('si-ph-input'); if (el) el.focus();
}
...
def _signup_step(phone: str = "", email: str = "", error: str | None = None) -> FT:
    return Div(cls="wz-step", id="wz-signup", style="display:none")(
        _progress_bar(1),

        Div(cls="wz-step-header")(
            Div(cls="wz-hero-icon")("✨"),
            H1("Create account", cls="auth-title"),
            P("Fill in your details to get started.", cls="auth-sub"),
        ),
        ...
        Form(
            ...
            Div(cls="form-group")(
                Label("Mobile Number", cls="form-label"),
                Div(cls="input-icon-wrap", id="su-ph-wrap")(
                    Span("+63", cls="input-prefix"),
                    Input(
                        type="tel", name="phone", id="su-ph-input",
                        placeholder="917 123 4567",
                        cls="form-input has-prefix",
                        autocomplete="tel", inputmode="numeric",
                        value=phone,
                        oninput="validatePhone(this,'su-ph-wrap')",
                    ),
                    Div(cls="input-valid-icon")(_ico_check()),
                ),
            ),
            ...
            Div(cls="form-group")(
                Label("Email Address", cls="form-label"),
                Div(cls="input-icon-wrap", id="su-em-wrap")(
                    Div(cls="input-icon")(_ico_email()),
                    Input(
                        type="email", name="email", id="su-em-input",
                        placeholder="you@gmail.com",
                        cls="form-input",
                        autocomplete="email",
                        value=email,
                        oninput="validateEmailSU(this,'su-em-wrap')",
                    ),
                    Div(cls="input-valid-icon")(_ico_check()),
                ),
                P("OTP will be sent here. We never share your email.", cls="form-hint"),
            ),
            # PIN
            Div(cls="form-group")(
                Label("Choose a 4-digit PIN", cls="form-label",
                      style="text-align:center;display:block;"),
                Div(cls="otp-wrap", id="su-pin-boxes")(
                    *[
                        Input(
                            type="tel", maxlength="1",
                            cls="otp-input su-pin-input",
                            id=f"su-pin-{i}",
                ...
                P("Used to confirm payments. Keep it secret.", cls="form-hint",
            ...
            Div(cls="form-group")(
                Label("Confirm PIN", cls="form-label",
                      style="text-align:center;display:block;"),
                Div(cls="otp-wrap", id="su-pinc-boxes")(
                    *[
                        Input(
                            type="tel", maxlength="1",
                            cls="otp-input su-pinc-input",
                            id=f"su-pinc-{i}",
                            inputmode="numeric", pattern="[0-9]",
                        )
                        for i in range(4)
                ...
                P(id="su-pin-hint", cls="form-hint",
                  style="text-align:center;min-height:1.2em;margin-top:8px;"),
            ),
            # Hidden PIN values
            Input(type="hidden", name="pin",         id="su-pin-hidden"),
            Input(type="hidden", name="pin_confirm",  id="su-pinc-hidden"),
        ...
        Script("""
function validateEmailSU(inp, wrapId) {
  var wrap = document.getElementById(wrapId);
  var ok = /^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$/.test(inp.value.trim());
  if (wrap) wrap.classList.toggle('is-valid', ok);
  checkSUReady();
...
(function(){
  var pinInputs  = document.querySelectorAll('.su-pin-input');
  var confInputs = document.querySelectorAll('.su-pinc-input');
  var pinHidden  = document.getElementById('su-pin-hidden');
  var confHidden = document.getElementById('su-pinc-hidden');
  var submit     = document.getElementById('su-submit');
  var hint       = document.getElementById('su-pin-hint');

  function attachBoxes(inputs, hiddenEl) {
  ...
  function checkMatch() {
    var p = pinHidden ? pinHidden.value : '', c = confHidden ? confHidden.value : '';
    if (!hint) return;
    if (p.length < 4 || c.length < 4) { hint.textContent=''; checkSUReady(); return; }
    if (p === c) {
      hint.textContent = '✓ PINs match'; hint.style.color = 'var(--jade-light)';
    } else {
      hint.textContent = '✗ PINs do not match'; hint.style.color = 'var(--danger)';
    }
    checkSUReady();
  }

  window.checkSUReady = function() {
    if (!submit) return;
    var ph = document.getElementById('su-ph-input');
    var em = document.getElementById('su-em-input');
    var phOk = ph && ph.value.replace(/\\D/g,'').length >= 9;
    var emOk = em && /^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$/.test(em.value.trim());
    var pinOk = pinHidden && confHidden && pinHidden.value.length === 4 && pinHidden.value === confHidden.value;
    submit.disabled = !(phOk && emOk && pinOk);
  };

  attachBoxes(pinInputs, pinHidden);
  attachBoxes(confInputs, confHidden);

  /* also re-check when phone changes */
  var ph = document.getElementById('su-ph-input');
  if (ph) ph.addEventListener('input', checkSUReady);
})();
        """),
    )


# ─── Backwards-compat: identifier form fragment ──────────────────────────────

def identifier_form_fragment(error: str | None = None) -> FT:
    """Returned by the ← Back button from OTP step — shows choice + forms."""
    return Div()(
        _choice_step(),
        _signin_step(error=error),
        _signup_step(),
    )


def signup_form_fragment(phone: str = "", email: str = "", error: str | None = None) -> FT:
    """Returned when a sign-up error needs to be shown after OTP bounce."""
    return Div()(
        _choice_step(),
        _signin_step(),
        _signup_step(phone=phone, email=email, error=error),
        Script("showSignUp();"),
    )
...
def otp_step(masked_email: str, email: str, error: str | None = None,
             _phone: str = "", _pin: str = "") -> FT:
    return Div(
        cls="wz-step",
        ...
        Div(cls="wz-step-header")(
            ...
            P(
                ...
                cls="auth-sub", style="margin-bottom:4px",
            ),
        ...
        Form(
            ...
            Input(type="hidden", name="pin",   value=_pin)   if _pin   else None,
...
# ─── Step 3 — PIN creation (legacy fallback only) ─────────────────────────────
        ...
        Form(
            ...
            Div(cls="form-group")(
                ...
                Div(cls="otp-wrap", id="pin-boxes")(
                    *[Input(type="tel", maxlength="1", cls="otp-input pin-input",
                            id=f"pin-{i}", inputmode="numeric", pattern="[0-9]")
                      for i in range(4)]
                ),
            ),
            Div(cls="form-group")(
                ...
                Div(cls="otp-wrap", id="pin-confirm-boxes")(
                    *[Input(type="tel", maxlength="1", cls="otp-input pin-confirm-input",
                            id=f"pinc-{i}", inputmode="numeric", pattern="[0-9]")
                      for i in range(4)]
                ),
            ...
            Button(
                _ico_lock(), Span("Create Account"), Span(cls="htmx-indicator"),
                type="submit", id="pin-submit",
                cls="btn btn-primary btn-block",
...
.wz-dot-done   { background: var(--primary, #0D9488); color: #fff; }
.wz-dot-active { background: var(--jade-light, #14B8A6); color: #fff;
...
@media (max-width: 480px) {
  .wz-choice-row { grid-template-columns: 1fr; gap: 10px; }
}
.wz-choice-card {
...
.wz-choice-card:active { transform: translateY(0); }
.wz-choice-card-signup:hover {
  border-color: var(--jade-light, #14B8A6);
  background: rgba(20,184,166,0.07);
}
.wz-choice-icon { font-size: 1.6rem; flex-shrink: 0; line-height: 1; }
.wz-choice-body { flex: 1; min-width: 0; }
.wz-choice-title { font-weight: 800; font-size: 0.95rem; color: var(--text); }
.wz-choice-desc  { font-size: 0.75rem; color: var(--muted); margin-top: 2px; }
.wz-choice-card svg { flex-shrink: 0; color: var(--muted); }
.wz-choice-card:hover svg { color: var(--primary, #0D9488); }

[data-theme="light"] .wz-choice-card { background: #fff; }
[data-theme="light"] .wz-choice-card:hover { background: rgba(13,148,136,0.05); }
...
.wz-alt-link  {
  ...
  font-size: 0.8rem; color: var(--jade-light, #14B8A6); font-weight: 600;
...
.wz-trust-icon { color: var(--jade-light, #14B8A6); }
