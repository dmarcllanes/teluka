...
// Show permission prompt banner after 4 s if not yet decided
(function () {
  ...
  setTimeout(function () {
    ...
    banner.innerHTML =
      ...
      + '<span class="push-banner-text">Get deal alerts even when app is closed</span>'
      + '<button class="push-banner-allow" onclick="telukaPushEnable()">Allow</button>'
      + '<button class="push-banner-dismiss" onclick="telukaPushDismiss()" aria-label="Close">✕</button>';
    document.body.appendChild(banner);
    requestAnimationFrame(function () { banner.classList.add('push-banner-visible'); });
  }, 4000);
})();

async function telukaPushEnable() {
  ...
  if (perm === 'granted') {
    ...
    if (ok) {
      var el = document.getElementById('notif-toggle-label');
      if (el) el.textContent = 'Notifications On ✓';
    }
  }
}

function telukaPushDismiss() {
  sessionStorage.setItem('push-dismissed', '1');
  var banner = document.getElementById('push-banner');
  if (banner) { banner.classList.remove('push-banner-visible'); setTimeout(function(){ banner.remove(); }, 300); }
}
